...
// Clone set 3 times for infinite loop
const extendedVehicles = [...vehicles, ...vehicles, ...vehicles];

export default function Stock() {
  const [currentIndex, setCurrentIndex] = useState(N); // Start at middle set (index 6)
  ...
  // Reset transition after instant position swap
  useEffect(() => {
    if (!isTransitioning) {
  ...
  const goToDot = (idx: number) => {
    setIsTransitioning(true);
    const currentSetBase = Math.floor(currentIndex / N) * N;
    setCurrentIndex(currentSetBase + idx);
  };
        ...
        >
          ...
          >
            {extendedVehicles.map((v, i) => (
              ...
              >
                <div className="bg-[#FFFFFF] border border-[#E5E0D8] rounded-[8px] p-5 flex flex-col justify-between h-full group hover:-translate-y-1 hover:border-[#C41E1E] transition-all duration-300">
                  <div>
                    {/* Placeholder Photo */}
                    <div className="bg-[#F0EDE8] aspect-[16/10] rounded-[6px] mb-5 relative flex items-center justify-center overflow-hidden">
                      {v.type === "Carro" ? (
                        ...
                        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#C9C0B4" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round">
                          ...
                          <path d="M15 6h2l2 4" />
                          <path d="M12 17.5V14l-3-3 4-3 2 3h3.5" />
                        </svg>
                      )}
                    ...
                    <h3 className="font-['Cormorant_Garamond'] font-semibold text-[18px] text-[#1A1A1A] mb-1">
                      {v.name}
                    </h3>
                    <p className="font-['DM_Sans'] font-light text-[12px] text-[#9B8E7E] mb-4">
                      {v.km} · {v.year}
                    </p>
                  </div>

                  <div>
                    <div className="font-['Playfair_Display'] font-bold italic text-[1.3rem] text-[#8B7355] mb-4">
                      {v.price}
                    </div>

                    <a
                      href={`https://wa.me/5521969320071?text=${encodeURIComponent(
                        `Olá! Tenho interesse no ${v.name} (${v.year}) anunciado por ${v.price}.`
                      )}`}
                      target="_blank"
          ...
          {Array.from({ length: N }).map((_, idx) => (
            <button
              ...
              className={`transition-all duration-300 cursor-pointer ${
                activeDotIndex === idx
                  ? "w-[18px] h-[6px] bg-[#C41E1E] rounded-full"
                  : "w-[6px] h-[6px] bg-[#E5E0D8] rounded-full hover:bg-[#C9C0B4]"
              }`}
            />
          ))}
        </div>
